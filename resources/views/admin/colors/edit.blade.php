...
@section('content')
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg overflow-x-hidden">
        ...
        <div class="row backgroundW p-4 m-3">
            ...
            <form action="{{route('update.colors', $colors->id)}}" method="POST">
                @csrf
                @method('PUT')
                @if (count($errors) > 0)
                    <ul>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-controll" name="name" value="{{$colors->name}}">
                </div>
                <div class="form-group">
                    <label for="exampleFormControlInput1">رمز لون</label>
                    <input type="text" class="form-controll" name="code" value="{{$colors->code}}">
                </div>
                {{-- <div class="form-group">
                ...
                <div class="form-check form-check-inline">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" {{$colors->status == 1 ? 'checked' : ''}}>
                </div>
